...
public class EntityPregnantSheep extends EntitySheep implements IViviparous,IEntityAdditionalSpawnData
{
    ...
    public EntityPregnantSheep(World world)
    {
        super(world);
    }
    ...
    {
    	EntityAnimal entityanimal2=new EntitySheep(worldObj);
    	if (this.Embyos==null){
    		this.setDead();
    	...
    	if (this.EmbyoProgress==this.EmbyoGrowTime){
    		...
    		((EntitySheep)entityanimal2).setFleeceColor(this.getFleeceColor());
    		((EntitySheep)entityanimal2).setSheared(this.getSheared());
    		switch (this.Embyos){
    		case Pig:
